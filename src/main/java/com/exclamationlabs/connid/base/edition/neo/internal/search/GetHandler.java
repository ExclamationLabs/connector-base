...
public class GetHandler<T extends ConnectorConfiguration> {
    ...
    public void get(GetType getType, T configuration,
    ...
    private GetStrategy deduceStrategy(GetType getType, T configuration, Class<? extends IdentityModel> identityModelClass, Filter queryFilter) {
    ...
    private void getAll(T configuration, Driver<T> driver,
        ...
        if (invocator == null) {
            results = driver.getAll(configuration, strategy.getIdentityModelClass(), resultsFilter,
                    resultPaginator, -1, Collections.emptyMap());
        } else {
            results = (Set<IdentityModel>) invocator.getAll(driver, configuration, resultsFilter,
                    resultPaginator, -1, Collections.emptyMap());
        }
        if (results != null) {
