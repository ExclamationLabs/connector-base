...
/**
 * Connector that allows write-only access on a destination system. This class should be subclassed
 * if your primary desire is to have just write-only access to the data types on the destination
 * system.
 *
 * <p>Note that only the ConnId SearchOp&lt;String&gt; interface is not included for this class;
 ...
 */
public abstract class BaseWriteOnlyConnector<T extends ConnectorConfiguration>
    extends BaseConnector<T> implements DeleteOp, CreateOp, UpdateDeltaOp {

  public BaseWriteOnlyConnector(Class<T> configurationTypeIn) {
    super(configurationTypeIn);
  }

  public BaseWriteOnlyConnector(Class<T> configurationTypeIn, boolean commonsLogging) {
